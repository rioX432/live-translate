...
社内の日本人・外国人混在環境で、All Hands Meeting等のプレゼン中にMac拡張ディスプレイ上でリアルタイム字幕翻訳を表示する。日本語↔英語の自動切替が必要。文字起こしログも残したい。
...
- 文字起こしログ保存
- なるべくコストをかけない
- Claude API Keyは使わない（各メンバーがサブスクを持っている）
- 既存ツール: Google Meet, Slack, Notion, CircleBack
