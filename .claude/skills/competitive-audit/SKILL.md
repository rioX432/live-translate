...
description: "Core Value-filtered competitive analysis: research user pain points, identify gaps within Core Value scope, file at most 3 high-impact GitHub Issues and record the rejected ideas as Won't Do. Requires the gh CLI."
...
Read [research-method.md](research-method.md) and run its Phase 2 sections: commercial competitors (2a),
OSS competitors (2b), **user pain point synthesis (2c)**, UX quality comparison (2d), and store review
analysis (2e), under the research rules stated there.

2c is the load-bearing one. A competitor feature with no matching user pain point is not a gap — it is
their roadmap, not ours, and Phase 3c will reject it.
...
Read [research-method.md](research-method.md) → **Phase 4a: Improvement research categories** and research
each disadvantage that survived Phase 3c against those categories.
...
Hand the selected candidates to the `issue` skill — it owns the sizing gate, splitting, and the body template:

```
Skill("issue", args: "Batch: competitive audit candidates (max 3).
Source: /competitive-audit run on {focus area}.
{for each candidate: title, Core Value alignment with one-step reasoning,
user pain point evidence, complexity cost, priority label, reference URLs}")
```

Competitive-audit-specific requirements to pass through:

- **Maximum 3 issues per audit run** — this cap overrides the batch default of 10
- The `Context` section must carry USER PAIN POINT evidence, not "competitor has it"
- Complexity cost (new dependencies, config surface, maintenance burden) goes in `Risks / Notes`
- A candidate whose scope spans several releases becomes an epic with children, never one issue
- Add the `research` label to items still requiring investigation
...
- If issue filing fails, the `issue` skill outputs the drafted bodies as markdown for manual creation
